...
- **交互式错误恢复** - 遇到构建或测试失败时，提供三选项菜单：继续尝试、手动修复（vim编辑器）或退出，支持错误详情显示和自动验证
...
当遇到构建或测试失败时，工具不会立即退出，而是提供一个交互式菜单，让您选择如何继续：

```
│
│ ⚠ Build failed!
│ File example.rs still has build errors after 5 fix attempts.
│
│ error[E0425]: cannot find value `x` in this scope
│  --> src/example.rs:10:5
│
│ Please choose how to proceed:
│ 1. Continue trying - Retry translation from scratch
│ 2. Manual fix - Edit the file manually
...
1. **Continue trying（继续尝试）**
   - 对于构建失败：重新从头开始翻译该文件
   - 对于测试失败：提供修复建议，工具会结合您的建议和错误信息进行修复

2. **Manual fix（手动修复）**
   - 优先尝试使用 vim 编辑器打开文件
   - 如果 vim 不可用，显示文件的绝对路径供您使用其他编辑器修复
   - 修复后工具会自动验证（重新构建或运行测试）
   - 如果验证失败，会显示错误详情并重新显示菜单
...
**场景1：构建失败**
- 发生时机：达到最大修复尝试次数（默认10次）后仍有构建错误
- 建议操作：选择"Manual fix"手动修正复杂的类型转换或语法问题

**场景2：测试失败**
- 发生时机：文件构建成功但混合构建测试失败
- 建议操作：选择"Continue trying"并提供具体的修复建议（如"修改函数返回值类型"）

**场景3：初始化阶段失败**
- 发生时机：启动翻译时的初始构建或测试失败
- 建议操作：选择"Manual fix"检查项目配置或依赖问题

所有错误恢复菜单都会显示截断后的错误详情（默认前20行），帮助您快速了解问题所在。
