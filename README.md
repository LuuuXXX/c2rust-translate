...
- **进度跟踪** - 使用 `calculate_progress()` 实时计算翻译进度（公式：`(total - empty) / total`），显示当前处理文件在所有文件中的序号和总数（如 `[7/10] Processing var_example.rs`）
- **智能恢复** - 支持 Ctrl+C 中断后从上次位置恢复：
  - 每个文件完成后自动保存进度到 `translation_stats.json`
  - Ctrl+C 触发 SIGINT 处理程序，设置中断标志并保存当前进度
  - 重新运行时提供两种选择：**继续上次进度** 或 **重新开始**
...
- `lib` - 主要翻译工作流程编排
- `util` - 通用工具函数（进度计算、统计、中断处理等）

### 关键工具函数（util 模块）

| 函数 | 说明 |
|------|------|
| `find_project_root()` | 从当前目录向上搜索 `.c2rust` 目录以确定项目根 |
| `validate_feature_name()` | 验证功能名称防止路径遍历攻击 |
| `calculate_progress(total, empty)` | 计算翻译进度比例 `(total - empty) / total` |
| `is_interrupted()` | 检查是否收到 Ctrl+C 中断信号 |
| `set_interrupted()` | 设置中断标志（由信号处理程序调用） |
