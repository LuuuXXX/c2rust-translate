...
### 步骤 3：扫描要翻译的文件并初始化进度跟踪
- 扫描 rust 目录中的所有 `.rs` 文件
- 计算已处理和未处理的文件数量
- 初始化进度跟踪状态
...
- 扫描空的 `.rs` 文件（未处理的文件）
- 选择要处理的文件：
  - 交互式模式：提示用户选择文件（范围、单个或"全部"）
  - 自动模式：使用 `--allow-all` 标志处理所有文件
- 按顺序处理每个选定的文件
- 对于每个文件：
  1. 将 C 翻译为 Rust
...
### 步骤 6：告警修复阶段
- 所有翻译任务完成后自动触发
- 提示用户：通过列表选择是否进入告警修复阶段（使用方向键选择"是，进入告警修复阶段"或"否，跳过告警修复"）
- 若用户选择"是"：
  - 扫描所有已翻译的非空 `.rs` 文件，并准备进行统一检查
  - 通过单次 crate 级构建执行告警检测和修复循环（最多 `max_fix_attempts` 次），覆盖所有相关文件
- 若用户选择"否"：直接退出，跳过告警修复
