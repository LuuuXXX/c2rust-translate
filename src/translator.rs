    ...
    match std::fs::read_to_string(file_path) {
        Ok(content) => {
            ...
            let display_lines = if show_full {
                ...
                std::cmp::min(total_lines, max_lines)
            };
...
fn create_error_temp_file(error_msg: &str) -> Result<tempfile::NamedTempFile> {
    let mut temp_file =
        tempfile::NamedTempFile::new().context("Failed to create temporary error file")?;
    write!(temp_file, "{}", error_msg).context("Failed to write error message to temp file")?;
    Ok(temp_file)
}
...
mod tests {
    ...
    fn test_build_fix_args() {
        ...
        let args_with_suggestion = build_fix_args(
            script,
            config,
            c_code,
            rust_code,
    ...
    fn test_display_code_truncation_behavior() {
        ...
        assert_eq!(
            display_lines, 10,
            "Should truncate to max_lines when show_full is false"
        );
        assert!(
        ...
        assert_eq!(
            display_lines, 20,
            "Should display all lines when show_full is true"
        );
        assert_eq!(
            total_lines, display_lines,
    ...
    fn test_display_code_no_truncation_when_lines_less_than_max() {
        ...
        assert_eq!(
            display_lines, 5,
            "Should display all lines when total < max_lines"
        );
        assert_eq!(
            total_lines, display_lines,
            "Should not show truncation message"
    ...
    fn test_display_error_preview_truncation_behavior() {
        ...
        assert_eq!(
            display_lines, 25,
            "Should display all error lines when show_full is true"
        );
        assert_eq!(
            total_lines, display_lines,
            "Should not show truncation message"
    ...
    fn test_display_error_preview_no_truncation_when_lines_less_than_max() {
        ...
        assert_eq!(
            display_lines, 3,
            "Should display all error lines when total < ERROR_PREVIEW_LINES"
        );
        assert_eq!(
            total_lines, display_lines,
            "Should not show truncation message"
