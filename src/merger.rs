...
///
/// 关键处理：若合并输出中存在 glob 导入（如 `use core::ffi::*;`），
/// 则过滤掉同模块下已被 glob 覆盖的显式导入（如 `use core::ffi::c_int;`），
/// 避免在 `#![deny(unused_imports)]` 下产生编译错误。
/// glob 导入本身永远不会被删除。
fn merge_files(files: &[(PathBuf, String)]) -> MergeResult {
    ...
    // glob 导入位于 complex_uses 中，永远不会被过滤删除。
    ...
    // 注意：此过滤仅作用于 sorted_named（简单导入），不影响 complex_uses（含 glob）。
...
/// - 若输入中存在 glob 导入（如 `use core::ffi::*;`），该 glob 始终被保留；
///   同模块下被 glob 覆盖的显式导入（如 `use core::ffi::c_int;`）会被丢弃，
///   以避免在 `#![deny(unused_imports)]` 下产生编译错误
///
/// # 参数
/// - `feature`: feature 名称
...
pub fn merge_feature(feature: &str, output_path: Option<&Path>) -> Result<PathBuf> {
    ...
    let all_rs = file_scanner::find_translated_rs_files(&rust_src_dir).with_context(|| {
        format!(
    ...
    fs::write(&resolved_output, &output_content).with_context(|| {
        format!(
...
mod tests {
    ...
    #[test]
    fn test_glob_module_prefix_core_ffi() {
        assert_eq!(
            glob_module_prefix("use core::ffi::*;"),
            Some("core::ffi".to_string())
        );
    }

    #[test]
    fn test_glob_module_prefix_std_ffi() {
        assert_eq!(
            glob_module_prefix("use std::ffi::*;"),
            Some("std::ffi".to_string())
        );
    }

    #[test]
    fn test_glob_module_prefix_non_glob() {
        assert_eq!(glob_module_prefix("use core::ffi::c_int;"), None);
    }

    #[test]
    fn test_glob_module_prefix_compound() {
        assert_eq!(glob_module_prefix("use std::{ffi, fmt};"), None);
    }

    // ── is_covered_by_glob ────────────────────────────────────────────────

    #[test]
    fn test_is_covered_by_glob_simple() {
        assert!(is_covered_by_glob("use core::ffi::c_int;", "core::ffi"));
    }

    #[test]
    fn test_is_covered_by_glob_different_module() {
        assert!(!is_covered_by_glob(
            "use std::collections::HashMap;",
            "core::ffi"
        ));
    }
    ...
    #[test]
    fn test_is_covered_by_glob_nested_not_covered() {
        // use core::ffi::sub::item should NOT be covered by core::ffi glob
        // (sub::item has a :: so it's a nested path)
        assert!(!is_covered_by_glob(
            "use core::ffi::sub::item;",
            "core::ffi"
        ));
    }
    ...
    fn test_merge_preserves_c_int_declaration() {
        ...
        assert_eq!(
            c_int_count, 1,
            "c_int should appear exactly once in merged use declarations"
        );
    ...
    fn test_merge_complex_uses_preserved() {
        ...
        assert_eq!(
            complex_count, 1,
            "identical complex use should be deduplicated"
        );
    }
    ...
    fn test_merge_glob_does_not_produce_duplicate_explicit_import() {
        ...
        assert_eq!(
            result.use_declarations[0].trim_end_matches(';'),
            "use core::ffi::*"
        );
    }
    ...
    fn test_merge_explicit_from_different_module_not_dropped() {
        ...
        assert!(
            has_hashmap,
            "HashMap from a different module must be preserved"
        );
    }
