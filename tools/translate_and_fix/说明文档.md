## 各文件功能概览
- config.toml: 存放大模型配置信息（位于 .c2rust/config.toml）
...
### 配置文件位置
工具会自动从项目根目录下的 `.c2rust/config.toml` 读取配置。
- 项目根目录为 `tools` 目录的父目录
...
### 调用变量翻译工具
调用示例：
`python translate_and_fix.py --type var --code code.c --output output.rs`

### 调用函数翻译工具
调用示例：
`python translate_and_fix.py --type fn --code code.c --output output.rs`

### 调用语法修复工具
调用示例：
`python translate_and_fix.py --type fix --code code.rs --output output.rs --error error.txt`

## 各文件功能说明
### config.toml (位于 .c2rust/config.toml)
在这里存放大模型的配置信息，这些信息用于工具初始化时创建与大模型平台的api调用会话，从而在翻译过程中调用大模型。配置文件使用TOML格式，主要包括以下配置项：
...
### LLM_calling.py
#### class LLM_config
用于存放从 `config.toml` 读取的配置信息，各成员变量即为 `config.toml` 中 `[model]` 段的各配置项。

#### load_LLM_config(config_file_path)
根据入参`config_file_path`（即 `config.toml` 文件的路径）读取大模型配置信息，并将它们存放到一个`LLM_config`对象中。
